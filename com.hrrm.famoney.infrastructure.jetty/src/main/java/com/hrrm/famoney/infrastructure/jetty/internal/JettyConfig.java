...
public final class JettyConfig {
    /**
     * Felix specific property to set the interface to listen on. Applies to both
     * HTTP and HTTP
     */
    ...
    /**
     * Felix specific property to control whether to want or require HTTPS client
     * certificates. Valid values are "none", "wants", "needs". Default is "none".
     */
    public static final String FELIX_HTTPS_CLIENT_CERT = "org.apache.felix.https.clientcertificate";

    /**
     * Felix specific property to configure the session timeout in minutes (same
     * session-timout in web.xml). Default is servlet container specific
     */
    ...
    /**
     * Felix specific property to configure the request buffer size. Default is 16KB
     * (instead of Jetty's default of 4KB)
     */
    ...
    /**
     * Felix specific property to enable Jetty MBeans. Valid values are "true",
    ...
    /**
     * Felix specific property to configure the excluded cipher suites. @deprecated
     * use {@link #FELIX_JETTY_EXCLUDED_SUITES} instead.
     */
    @Deprecated
    ...
    /**
     * Felix specific property to configure the included cipher suites. @deprecated
     * use {@link #FELIX_JETTY_INCLUDED_SUITES} instead.
     */
    @Deprecated
    ...
    /**
     * Felix specific property to specify whether a server header should be sent
     * (defaults to true)
     */
    ...
    /**
     * Felix specific properties to be able to disable renegotiation protocol for
     * TLSv1
     */
    public static final String FELIX_JETTY_RENEGOTIATION_ALLOWED = "org.apache.felix.https.jetty.renegotiateAllowed";

    /**
     * Felix specific property to control whether to enable Proxy/Load Balancer
     * Connection
     */
    ...
    /**
     * Felix specific property to configure whether JSESSIONID parameter will be
     * added when encoding external URLs
     */
    ...
    /**
     * Felix specific property to specify the published "name" property of the OSGi
     * Log Service-base Request Log service. Allows server configs to filter on
    ...
    /**
     * Felix specific property to control the level of the log messages generated by
     * the OSGi Log Service-based request log. Values must correspond to the
     * constants defined in the LogService interface, default is 3 "INFO".
     */
    ...
    /**
     * Felix specific property to specify the published "name" property of the
     * file-based RequestLog service. Allows server configs to filter on specific
     * log services.
     */
    ...
    /**
     * Felix specific property to enable request logging to append to the log file
     * rather than overwriting
     */
    public static final String FELIX_HTTP_REQUEST_LOG_FILE_APPEND = "org.apache.felix.http.requestlog.file.append";

    /**
     * Felix specific property to specify the number of days the request log file is
     * retained
     */
    ...
    /**
     * Felix specific property to enable request logging cookies in the request log
     * file
     */
    ...
    /**
     * Felix specific property to enable request logging request processing time in
    ...
    /**
     * Felix specific property to define custom properties for the http runtime
     * service.
     */
    ...
    /**
     * Felix specific property to specify the minimum response size to trigger
     * dynamic compression
     */
    public static final String FELIX_JETTY_GZIP_MIN_GZIP_SIZE = "org.apache.felix.jetty.gzip.minGzipSize";

    /**
     * Felix specific property to specify the compression level to use to initialize
    ...
    /**
     * Felix specific property to specify the size in bytes of the buffer to inflate
     * compressed request, or 0 for no inflation.
     */
    ...
    /**
     * Felix specific property to specify the regular expressions matching user
     * agents to exclude
     */
    ...
    /**
     * Felix specific property to specify the path specs to include. Inclusion takes
     * precedence over exclusion.
     */
    ...
    /**
     * Felix specific property to specify the included mime types. Inclusion takes
     * precedence over exclusion.
     */
    ...
    public static final java.lang.String PROP_INVALIDATE_SESSION = "org.apache.felix.http.session.invalidate";

    public static final boolean DEFAULT_INVALIDATE_SESSION = true;

    public static final java.lang.String PROP_UNIQUE_SESSION_ID = "org.apache.felix.http.session.uniqueid";
    ...
    /**
     * Properties from the configuration not matching any of the predefined
     * properties. These properties can be accessed from the getProperty* methods.
     * <p>
     * This map is indexed by String objects (the property names) and the values are
     * just objects as provided by the configuration.
     */
    ...
    /**
     * Returns the named generic configuration property from the configuration or
     * the bundle context. If neither property is defined return the defValue.
     */
    ...
    public String getClientcert() {
        return getProperty(FELIX_HTTPS_CLIENT_CERT,
                "none");
    }

    public String getContextPath() {
        return validateContextPath(getProperty(FELIX_HTTP_CONTEXT_PATH,
                null));
    }

    public String[] getExcludedCipherSuites() {
        return getStringArrayProperty(FELIX_JETTY_EXCLUDED_SUITES,
                getStringArrayProperty(FELIX_JETTY_EXCLUDED_SUITES_OLD,
                        null));
    }

    public String[] getIncludedProtocols() {
        return getStringArrayProperty(FELIX_JETTY_INCLUDED_PROTOCOLS,
                null);
    }

    public String[] getExcludedProtocols() {
        return getStringArrayProperty(FELIX_JETTY_EXCLUDED_PROTOCOLS,
                null);
    }

    public int getHeaderSize() {
        return getIntProperty(FELIX_JETTY_HEADER_BUFFER_SIZE,
                16 * 1024);
    }

    public String getHost() {
        return getProperty(FELIX_HOST,
                null);
    }

    public int getHttpPort() {
        if (httpPort == null) {
            httpPort = determinePort(String.valueOf(getProperty(HTTP_PORT)),
                    8080);
        }
        return httpPort;
    }
    ...
    public int getHttpsPort() {
        if (httpsPort == null) {
            httpsPort = determinePort(String.valueOf(getProperty(HTTPS_PORT)),
                    8443);
        }
        return httpsPort;
    }
    ...
    public int getHttpTimeout() {
        return getIntProperty(HTTP_TIMEOUT,
                60000);
    }

    public String[] getIncludedCipherSuites() {
        return getStringArrayProperty(FELIX_JETTY_INCLUDED_SUITES,
                getStringArrayProperty(FELIX_JETTY_INCLUDED_SUITES_OLD,
                        null));
    }
    ...
    public String getKeystoreType() {
        return getProperty(FELIX_KEYSTORE_TYPE,
                KeyStore.getDefaultType());
    }

    public String getPassword() {
        return getProperty(FELIX_KEYSTORE_PASSWORD,
                this.context.getProperty(OSCAR_KEYSTORE_PASSWORD));
    }

    public String[] getPathExclusions() {
        return getStringArrayProperty(FELIX_HTTP_PATH_EXCLUSIONS,
                new String[] {
                        "/system"
                });
    }
    ...
    public int getThreadPoolMax() {
        return getIntProperty(FELIX_JETTY_THREADPOOL_MAX,
                -1);
    }

    public int getAcceptors() {
        return getIntProperty(FELIX_JETTY_ACCEPTORS,
                -1);
    }

    public int getSelectors() {
        return getIntProperty(FELIX_JETTY_SELECTORS,
                -1);
    }

    public int getRequestBufferSize() {
        return getIntProperty(FELIX_JETTY_REQUEST_BUFFER_SIZE,
                8 * 1024);
    }

    public int getResponseBufferSize() {
        return getIntProperty(FELIX_JETTY_RESPONSE_BUFFER_SIZE,
                24 * 1024);
    }

    public int getMaxFormSize() {
        return getIntProperty(FELIX_JETTY_MAX_FORM_SIZE,
                200 * 1024);
    }
    ...
    public int getSessionTimeout() {
        return getIntProperty(FELIX_SESSION_TIMEOUT,
                0);
    }

    public String getTrustPassword() {
        return getProperty(FELIX_TRUSTSTORE_PASSWORD,
                null);
    }

    public String getTruststore() {
        String value = getProperty(FELIX_TRUSTSTORE,
                null);
        return value == null ||
            value.trim()
                .length() == 0 ? null : value;
    ...
    public String getTruststoreType() {
        return getProperty(FELIX_TRUSTSTORE_TYPE,
                KeyStore.getDefaultType());
    }

    public boolean isRegisterMBeans() {
        return getBooleanProperty(FELIX_HTTP_MBEANS,
                false);
    }

    /**
     * Returns <code>true</code> if HTTP is configured to be used (
     * {@link #FELIX_HTTP_ENABLE}) and the configured HTTP port ({@link #HTTP_PORT})
     * is higher than zero.
     */
    public boolean isUseHttp() {
        boolean useHttp = getBooleanProperty(FELIX_HTTP_ENABLE,
                true);
        return useHttp && getHttpPort() > 0;
    }

    public boolean isSendServerHeader() {
        return getBooleanProperty(FELIX_JETTY_SEND_SERVER_HEADER,
                false);
    }

    /**
     * Returns <code>true</code> if HTTPS is configured to be used (
     * {@link #FELIX_HTTPS_ENABLE}) and the configured HTTP port
     * ({@link #HTTPS_PORT}) is higher than zero.
     */
    public boolean isUseHttps() {
        boolean useHttps = getBooleanProperty(FELIX_HTTPS_ENABLE,
                getBooleanProperty(OSCAR_HTTPS_ENABLE,
                        false));
    ...
    public boolean isRenegotiationAllowed() {
        return getBooleanProperty(FELIX_JETTY_RENEGOTIATION_ALLOWED,
                false);
    }
    ...
    public String getRequestLogFilter() {
        return getProperty(FELIX_HTTP_REQUEST_LOG_FILTER,
                null);
    }
    ...
    public int getRequestLogOSGiLevel() {
        return getIntProperty(FELIX_HTTP_REQUEST_LOG_OSGI_LEVEL,
                3); // 3 == LogService.LOG_INFO
    }

    public String getRequestLogFilePath() {
        return getProperty(FELIX_HTTP_REQUEST_LOG_FILE_PATH,
                null);
    }
    ...
    public String[] getRequestLogFileIgnorePaths() {
        return getStringArrayProperty(FELIX_HTTP_REQUEST_LOG_FILE_IGNORE_PATHS,
                new String[0]);
    }
    ...
    public boolean isGzipHandlerEnabled() {
        return getBooleanProperty(FELIX_JETTY_GZIP_HANDLER_ENABLE,
                false);
    }

    public int getGzipMinGzipSize() {
        return getIntProperty(FELIX_JETTY_GZIP_MIN_GZIP_SIZE,
                GzipHandler.DEFAULT_MIN_GZIP_SIZE);
    }

    public int getGzipCompressionLevel() {
        return getIntProperty(FELIX_JETTY_GZIP_COMPRESSION_LEVEL,
                Deflater.DEFAULT_COMPRESSION);
    }

    public int getGzipInflateBufferSize() {
        return getIntProperty(FELIX_JETTY_GZIP_INFLATE_BUFFER_SIZE,
                -1);
    }

    public boolean isGzipSyncFlush() {
        return getBooleanProperty(FELIX_JETTY_GZIP_SYNC_FLUSH,
                false);
    }

    public String[] getGzipExcludedUserAgent() {
        return getStringArrayProperty(FELIX_JETTY_GZIP_EXCLUDED_USER_AGENT,
                new String[0]);
    }

    public String[] getGzipIncludedMethods() {
        return getStringArrayProperty(FELIX_JETTY_GZIP_INCLUDED_METHODS,
                new String[0]);
    }

    public String[] getGzipExcludedMethods() {
        return getStringArrayProperty(FELIX_JETTY_GZIP_EXCLUDED_METHODS,
                new String[0]);
    }

    public String[] getGzipIncludedPaths() {
        return getStringArrayProperty(FELIX_JETTY_GZIP_INCLUDED_PATHS,
                new String[0]);
    }

    public String[] getGzipExcludedPaths() {
        return getStringArrayProperty(FELIX_JETTY_GZIP_EXCLUDED_PATHS,
                new String[0]);
    }

    public String[] getGzipIncludedMimeTypes() {
        return getStringArrayProperty(FELIX_JETTY_GZIP_INCLUDED_MIME_TYPES,
                new String[0]);
    }

    public String[] getGzipExcludedMimeTypes() {
        return getStringArrayProperty(FELIX_JETTY_GZIP_EXCLUDED_MIME_TYPES,
                new String[0]);
    }

    public long getStopTimeout() {
        return getLongProperty(FELIX_JETTY_STOP_TIMEOUT,
                -1l);
    }
    ...
    public void setServiceProperties(Hashtable<String, Object> props) {
        ...
        props.put(HTTPS_PORT,
                Integer.toString(getHttpsPort()));
        props.put(FELIX_HTTP_ENABLE,
                Boolean.toString(isUseHttp()));
        props.put(FELIX_HTTPS_ENABLE,
                Boolean.toString(isUseHttps()));
        if (getHttpServiceName() != null) {
            props.put(FELIX_HTTP_SERVICE_NAME,
                    getHttpServiceName());
        }

        props.put(PROP_INVALIDATE_SESSION,
                getBooleanProperty(PROP_INVALIDATE_SESSION,
                        DEFAULT_INVALIDATE_SESSION));
        props.put(PROP_UNIQUE_SESSION_ID,
                getBooleanProperty(PROP_UNIQUE_SESSION_ID,
                        DEFAULT_UNIQUE_SESSION_ID));
    ...
    private void addCustomServiceProperties(final Hashtable<String, Object> props) {
        ...
        while (keys.hasMoreElements()) {
            ...
            if (key.startsWith(FELIX_CUSTOM_HTTP_RUNTIME_PROPERTY_PREFIX)) {
                props.put(key.substring(FELIX_CUSTOM_HTTP_RUNTIME_PROPERTY_PREFIX.length()),
                        this.config.get(key));
            }
        }
    }
     ...
     * @return <code>true</code> if the configuration was updated due to a changed
     *         value, or <code>false</code> if no change was found.
     */
     ...
     * </dl>
     *
     * @param portProp The port property value to parse.
    ...
    private int determinePort(String portProp, int dflt) {
        ...
        if ("*".equals(portProp) || "0".equals(portProp)) {
            ...
            if (
                ...
                String interval = portProp.substring(1,
                        portProp.length() - 1);
                int comma = interval.indexOf(',');

                // check if the comma is first (start port in range is missing)
                int start = (comma == 0) ? minPort
                        : parseInt(interval.substring(0,
                                comma),
                                minPort);
                // check if the comma is last (end port in range is missing)
                int end = (comma == interval.length() - 1) ? maxPort
                        : parseInt(interval.substring(comma + 1),
                                maxPort);
    ...
    /**
     * Get the property value as a string array. Empty values are filtered out - if
     * the resulting array is empty the default value is returned.
     */
